...
export const BaseView = props => {
    const {
        ...
        needScrollView, onRefresh, scrollEnabled,
    } = props
    const scrollViewProps = {onRefresh, scrollEnabled}
    const isDarkMode = useColorScheme() === 'dark';

    return <View style={{flex: 1}}>
        <Safe style={{flex: 0, backgroundColor: isDarkMode ? Color.main : statusBarBackColor}}/>
...
export const Button = props => {
    const {
        width, height, style, viewStyle, onPress, disabled, enablePressFadeIn, enablePressFadeOut,
        textColor, text,
        ...others
    } = props;
    const pressableProps = {onPress, disabled, enablePressFadeIn, enablePressFadeOut}
...
export const Image = props => {
    const {
        containerStyle: style, viewStyle, onPress, hitSlop, source, disabled,
        ...others
    } = props;
...
export const Pressable = props => {
    const {children, viewStyle, enablePressFadeIn, enablePressFadeOut, ...others} = props;
    ...
    return (
        ...
        >
            ...
            {modalType === 'slide'
                ...
                >
                    <Safe style={{backgroundColor:Color.mainBackground, borderTopRightRadius:50}}>
                        <Pressable
                            disabled={false}
                        ...
                        >
                            ...
                            >
                                <View style={[css.popmenu,modalInViewStyle]}>
                                    {onCloseIcon && <Icon name="close" size={23} color={Color.black} onPress={onCloseIcon}/>}
                                    {title !== undefined && (
                                        <Text
                                            center
                                            size={15}
                                    ...
                                    {/*{children}*/}
                                    {children ? children : Array.isArray(list) && list.map((item, i) => <View key={`pop${i}`}>
                                        <View style={{backgroundColor: Color.light, height: 1}}/>
                                        {renderItem ? renderItem(item, i) : <TouchableHighlight onPress={item.onPress} style={{padding: 15,}} viewStyle={[Style.inline, Style.center]}>
                                            {item.img && <Image source={item.img} style={{marginHorizontal: 10, width: 50, height: 50}}/>}
...
export const Text = props => {
    ...
    const textWithImg = img ? (
        <View style={[css.imageWithText, imgContainerStyle]}>
            <Image
                source={img}
                style={[Dim.margin(0, 5, 0, 0), imgStyle]}
            />
            {text}
        </View>
    ) : rightImg ? (
        <View style={[css.imageWithText, imgContainerStyle]}>
            ...
            <Image
                source={rightImg}
                style={[Dim.margin(0, 0, 0, 5), imgStyle]}
            />
        </View>
    ): (
...
export const TouchableHighlight = props => {
    const {children, onPress, underlayColor = 'transparent', viewStyle, ...others} = props;

    return <Touch underlayColor={underlayColor} onPress={typeof onPress === 'function' ? onPress : undefined} {...others}><View style={viewStyle}>{children}</View></Touch>
...
export const Sp = props => {
    ...
    return (
        <View style={[Dim.cssSize('100%', 1), Color.back(color), style]}/>
    )
}
...
export const RangeItem = () => {
    ...
    return <View style={{width}}>
        <View style={[Dim.cssSize('100%', Dim.y(10), Dim.y(10) / 2), Color.back(color)]}/>
        {isShowText === true && <View style={[Style.inline, Dim.padding(16.6), {justifyContent: 'space-between'}]}>
            <Text size={11} color="gray">{min}</Text>
            <Text size={11} color="gray">{max}</Text>
        </View>}
    </View>
}
...
export const RangeGraph = () => {
    ...
    const {color, min, max, value, valueMin, valueMax, isShowTriangle, isShowText} = this.props;
    ...
    return <View>
        ...
        <View style={[Style.inline, Style.fullAbs]}>
            ...
            <RangeItem width={`${(valueMax - valueMin) / (max - min) * 100}%`} min={valueMin} max={valueMax}
                       color={color} isShowText={isShowText}/>
        </View>
        <View style={[Style.fullAbs, {top: -Dim.y(barHeight)}]}>
            <View style={Style.inline}>
                <View style={{width: `${(value - min) / (max - min) * 100}%`, height: Dim.y(10)}}/>
                <View>
                    {isShowTriangle && <View style={{...Style.triangle(21, 18, 'top', Color.red), left: -Dim.x(9.5)}}/>}
                    <View style={[Dim.size(3.3, 46.6), Color.back('gray'), Dim.margin(5)]}/>
                </View>
            </View>
        </View>
    </View>
...
export const TouchableButton = (props) => {
    ...
    return <RectButton underlayColor={underlayColor} rippleColor={underlayColor} onPress={typeof onPress === 'function' ? onPress : undefined} {...others}><View style={viewStyle}>{children}</View></RectButton>
}
